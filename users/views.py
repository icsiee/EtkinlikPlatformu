...
def select_event(request, event_id):
    ...
    # Kullanıcının zaten katıldığı etkinlikleri kontrol et
    user = request.user

    # Kullanıcının katıldığı etkinliklerle tarih ve saat çakışmasını kontrol et
    conflicting_events = Event.objects.filter(
        date=event.date,
        time=event.time
    ).exclude(id=event.id)  # Şu anki etkinlik dışında diğer etkinlikleri kontrol et

    if conflicting_events.exists():
        # Çakışan etkinliklerin sayısı varsa, kullanıcıya uyarı mesajı gönder
        messages.warning(request,
                         'Seçtiğiniz etkinlik, başka bir etkinlik ile çakışmaktadır. Lütfen farklı bir etkinlik seçin.')
        return redirect('events_list')  # Etkinlikler listesine yönlendirebilirsiniz

    # Çakışma yoksa, etkinliği katılımcılar arasına ekle
    Participant.objects.create(user=user, event=event)
    messages.success(request, 'Etkinliğe başarıyla katıldınız!')

    return redirect('events_list')  # Katılım başarılı ise etkinlikler listesine yönlendir